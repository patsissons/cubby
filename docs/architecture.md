...
discovery site, all from the same source of truth (`cubby.json` files plus
`cubby.config.json`):

- **llms.txt** (llmstxt.org markdown): `pb_public/llms.txt` indexes every
  visible app plus the platform; each visible app gets
  `pb_public/<name>/llms.txt` built from its manifest (title, description,
  category, tags, and which cubby bundles its index.html loads). Generated
  files end with a `<!-- generated by cubby ... -->` marker comment; the
  build rewrites marked (or absent) files and never touches a file without
  the marker, so deleting the marker — or committing your own file first —
  opts an app into hand-authoring. Links are absolute when
  `config.domain`/`instanceUrl` is set, relative otherwise.
- **JSON-LD** (schema.org): the build injects one
  `<script type="application/ld+json" data-cubby-jsonld>` block per page —
  a WebSite + ItemList of visible apps on the root index.html, a
  WebApplication on each visible app's page — and replaces it in place on
  every run (append-before-`</head>` the first time). Hand-written JSON-LD
