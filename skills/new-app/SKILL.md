...
│                             category?, tags? }
└── llms.txt      generated by `npm run build` from cubby.json; keep its
                  trailing marker comment (the build refreshes marked files),
                  or delete the marker / write your own to hand-author it
```

The build also injects a `<script type="application/ld+json"
data-cubby-jsonld>` block (schema.org WebApplication) into the app's
index.html and refreshes it on every build; leave the attribute alone.
Hand-written JSON-LD without that attribute is never touched.
...
3. `npm run build` so `sites.json`, the llms.txt files, and the JSON-LD
   blocks are fresh (CI fails on drift), and confirm the app card shows on
   the discovery site at `/`.
